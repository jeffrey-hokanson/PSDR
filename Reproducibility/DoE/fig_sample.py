...
@memory.cache
def random_sample(domain, N, seed = 0):
	np.random.seed(seed)
	return psdr.random_sample(domain, N)

@memory.cache
def maximin(domain, N, L = None, seed = 0):
	np.random.seed(seed)
	return psdr.maximin_sample(domain, N, L = L)
 

if __name__ == '__main__':
	# Pick which function to use for testing
	if False:
