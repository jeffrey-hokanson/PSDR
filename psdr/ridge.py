...
if __name__ == '__main__':
	...
	#fX = pmap(f, args = [(x,) for x in X], progress = True, parallel = True)

	#basis = LegendreTensorBasis(2,5)	
	#U0s = [ orth(np.random.randn(X.shape[1], 2)) for i in range(10) ]
	#args = [ (np.copy(U0), X, fX, deepcopy(basis) )  for U0 in U0s]
	#kwargs = {'xtol':1e-10}
	#kwargs = {'disp': False, 'beta': 0.0001, 'ftol': -1, 'gtol': -1, 'maxiter': 500}
	#print isinstance(kwargs, dict) 
	#Ucrs = pmap(grassmann_gauss_newton, args = args,kwargs = kwargs, parallel = True, progress = True, desc = 'ridge fitting')
	#print "parallel"
	#Ucrs = parallel_grassmann_gauss_newton(U0s, X, fX, basis, parallel = True, progress = True, desc = 'ridge fitting', **kwargs)
	#print "non parallel"
	#Ucrs = parallel_grassmann_gauss_newton(U0s, X, fX, basis, parallel = False, progress = True, desc = 'ridge fitting', **kwargs)
	...
	pra = PolynomialRidgeApproximation(degree = 7, subspace_dimension = 1, n_init = 10,
		 beta = 1e-4, disp = False, maxiter = 500, ftol = -1, gtol = -1,)
