...
def writeConfigFile(parameters, def_file,path):
	...
	shutil.copy(path + '/def_NACA0012.cfg',path + '/def_NACA0012.cfg.bak')
	cfg_file = path + '/def_NACA0012.cfg'  
	with open(cfg_file, 'w+') as f:
		f.write(tempDefStr)


def generateParameters(parameters):
	numOfParams = len(parameters[0])
...
def workflowbash(parameters):
	cwd = os.getcwd()
	inv_cfg = 'inv_NACA0012.cfg'
	def_cfg = 'def_NACA0012.cfg'
	inv_def_cfg = 'inv_NACA0012_def.cfg'

	parameters = np.asarray(parameters)
	parameters_dict_form = generateParameters(parameters)
	ld_out = np.zeros((parameters.shape[0],2))
	mydir = os.path.join(os.getcwd(), datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
	os.mkdir(mydir)
	for iteration,row in enumerate(parameters_dict_form):
		try:

			path = os.path.join(cwd, str(iteration))
			os.mkdir(path)
			# os.mkdir()
			shutil.copy('mesh_out.su2', path)
			...
			with open(os.path.join(mydir, str(iteration), 'forces_breakdown.dat'), 'r+') as f:
					lines = f.readlines()
			...
			for line in lines:
					if re.search(cl_pattern, line) is not None:
						temp = line.split('Total CL:       ')
						# print(temp)
						temp = temp[1].split(' ')
						cl = float(temp[0].split(' ')[0])

					if re.search(cd_pattern,line) is not None:
						temp = line.split('Total CD:       ')
						temp = temp[1].split(' ')
						cd = float(temp[0].split(' ')[0])
						break;
